...
def generate():
    ...
    threading.Thread(target=run_pipeline, args=(job_id,video_path,artist_name,threshold,sec_before,sec_after), daemon=True).start()
...
def run_pipeline(job_id, video_path, artist_name, threshold, sec_before, sec_after):
    ...
    try:
        ...
        # STEP 2 — Detection drops
        job["steps"]["2"] = "running"
        log(job_id,"Detection des drops...","warn")
        drops = detect_drops(audio_path, threshold, job_id)
        job["drops"] = drops
        ...
        for i, drop_sec in enumerate(drops):
            ...
            cut_clip(video_path, start, duration, out_path, job_id)
            reel = {"index":i+1,"filename":out_path.name,
                    "timecode_start":sec_to_str(start),"timecode_drop":sec_to_str(drop_sec),
                    "duration":duration,"download_url":f"/download/{artist_dir.name}/{out_path.name}"}
            reels.append(reel)
        ...
        # STEP 4 — Export
        job["steps"]["4"] = "running"
        (artist_dir/"metadata.json").write_text(json.dumps({"artist":artist_name,"drops":len(drops),"reels":reels},ensure_ascii=False,indent=2))
...
def extract_audio(video_path, job_id):
    ...
    if r.returncode != 0:
        raise Exception("Audio extraction failed: " + r.stderr[-200:])
...
def cut_clip(video_path, start_sec, duration, out_path, job_id=None):
    tmp = out_path.with_suffix('.tmp.mp4')

    # ETAPE 1 : decoupe rapide par copie
    r1 = subprocess.run([
        "ffmpeg","-y",
        "-i",str(video_path),
        "-ss",str(start_sec),
    ...
    if r1.returncode != 0:
        raise Exception(f"Cut failed: {r1.stderr[-300:]}")

    # ETAPE 2 : recadrage 9:16 sur le petit clip
    # Obtenir les dimensions du clip temporaire
    probe = subprocess.run([
    ...
    try:
        ...
        # Si ffprobe echoue, utiliser les dimensions de la video source
        probe2 = subprocess.run([
            "ffprobe","-v","quiet","-print_format","json",
            "-show_streams","-select_streams","v:0",str(video_path)
        ], capture_output=True, text=True)
        info2 = json.loads(probe2.stdout)
        vw = int(info2["streams"][0]["width"])
        vh = int(info2["streams"][0]["height"])

    # Calcul crop 9:16 centre, dimensions paires
    if vw / vh > 9/16:
        cw = (int(vh * 9 / 16) // 2) * 2
        ch = (vh // 2) * 2
    else:
    ...
    if job_id: log(job_id, f"Crop: {vw}x{vh} -> {cw}x{ch} -> 1080x1920", "warn")
    ...
    if r2.returncode != 0:
        raise Exception(f"Crop failed: {r2.stderr[-400:]}")
