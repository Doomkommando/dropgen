...
def extract_audio(video_path, job_id):
    ...
    r = subprocess.run([
        ...
        "-vn","-ac","1","-ar","11025","-f","wav",str(out)
    ], capture_output=True, text=True)
    if r.returncode != 0:
        raise Exception("Audio failed: " + r.stderr[-200:])
    log(job_id,"Audio extrait.","ok")
    return out

def detect_drops(audio_path, sensitivity=0.6, job_id=None):
    if job_id: log(job_id,"Analyse Librosa...","warn")

    import librosa
    from scipy.signal import find_peaks

    # Charge en basse qualite pour economiser RAM
    ...
    # Onset strength focalisee sur les basses (kick techno)
    ...
    # Seuil adaptatif
    mean = np.mean(onset_smooth)
    std  = np.std(onset_smooth)
    multiplier = 3.0 - (sensitivity * 0.2)
    thresh = mean + std * multiplier

    # Detection des pics
    min_dist = int(sr / hop * 25)
    ...
    drops = sorted([int(t) for t in times if 60 <= t <= duration - 60])
