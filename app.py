...
def detect_drops(audio_path, sensitivity=0.6, job_id=None):
    ...
    if job_id: log(job_id, 'Chargement audio Librosa...', 'warn')
    # Charge en mono 11025Hz pour economiser memoire (moitie moins que 22050)
    y, sr = librosa.load(str(audio_path), sr=11025, mono=True)
    if job_id: log(job_id, f'Audio charge : {len(y)/sr:.0f}s a {sr}Hz', 'ok')
    hop = 512
    ...
    del y  # libere memoire immediatement
    drops = [int(librosa.frames_to_time(p, sr=sr, hop_length=hop)) for p in peaks]
    result = sorted([t for t in drops if 60 <= t <= duration - 60])
    if job_id: log(job_id, f'{len(result)} drops trouves sur {duration:.0f}s', 'ok')
    return result
